...
def enhance_image(image_bgr, repaint=True):
    ...
    color2mask: dict[TextColor, np.ndarray] = {
        color: get_image_mask_by_color(image_hsv, color)
        for color in TextColor
        if color != TextColor.OTHER
    }
...
def stitch_images(image_list=None, save_dir="data/54", repaint=False):
