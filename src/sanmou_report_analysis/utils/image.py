...
def save_image(image, save_path: Path | str, log=False):
...
def nms(
    match_results: list[MatchResult | tuple[MatchResult, dict]],
    threshold: int | np.ndarray,
) -> list[MatchResult | tuple[MatchResult, dict]]:
    """
    带临时属性的NMS实现
...
def match_icon(
    icon, image, mode=cv2.TM_CCOEFF_NORMED, threshold=0.8, nms_threshold=20
) -> list[MatchResult]:
    ...
    dx, dy = icon.shape[:2]
    match_result = [
        MatchResult((left, top, left + dy, top + dx), score)
        for top, left, score in zip(*loc, confidence, strict=False)
    ]
...
def match_icon_many(
    ...
    match_results_list = []
    for icon_bgr, t in zip(icon_bgr_list, threshold, strict=False):
...
def draw_boxes(image_bgr, box_list: list[BoundingBox], color=(0, 0, 255), thickness=2):
    ...
    for box in box_list:
        ...
        cv2.rectangle(new_image, (left, top), (right, bottom), color, thickness)
    return new_image
...
def are_images_matching(img1, img2, match_threshold=0.8, reprojection_threshold=5.0, min_matches=5):
    ...
    M, mask = cv2.estimateAffine2D(
        src_pts, dst_pts, method=cv2.RANSAC, ransacReprojThreshold=reprojection_threshold
    )
...
def find_template_matches(
    ...
    for scale_orig in search_scales:
        ...
        # 如果模板比图像大则跳过
        if (
            scaled_template.shape[0] > large_gray.shape[0]
            or scaled_template.shape[1] > large_gray.shape[1]
        ):
            continue
        ...
        if method in [cv2.TM_SQDIFF, cv2.TM_SQDIFF_NORMED]:
            loc = np.where(
                result <= (1.0 - threshold) if method == cv2.TM_SQDIFF_NORMED else threshold
            )
        else:
            loc = np.where(result >= threshold)
    ...
    indices = cv2.dnn.NMSBoxes(
        bboxes=rects, scores=scores, score_threshold=threshold, nms_threshold=nms_threshold
    )
...
def detect_matches_with_sift(
    ...
    # 将匹配点分组
    np.float32([kp_small[m.queryIdx].pt for m in good_matches]).reshape(-1, 1, 2)
    ...
    for _cluster_id, cluster_matches in clusters.items():
        ...
        if (
            width < 5
            or height < 5
            or width > large_img.shape[1] * 0.5
            or height > large_img.shape[0] * 0.5
        ):
            continue
...
def count_template_matches(template, source, threshold=0.7, scales=np.arange(0.5, 1.5, 0.1)):
    ...
    for scale in scales:
        ...
        # 找到匹配位置
        loc = np.where(result >= threshold)
        for pt in zip(*loc[::-1], strict=False):  # 切换坐标顺序
            # 存储匹配框的左上角和右下角坐标及得分
