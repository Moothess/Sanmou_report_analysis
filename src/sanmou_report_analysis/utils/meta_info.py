...
from sanmou_report_analysis.utils.data_structure import Country, TroopType, create_lazy_dict

# from utils
from sanmou_report_analysis.utils.image import count_template_matches, read_image
...
def match_hero_country(image):
    ...
    h, _s, _v = int(hsv_color[0]), int(hsv_color[1]), int(hsv_color[2])
...
def match_hero_troop_type(image):
    ...
    for target_name, target_img in _troop_type_icon_dict.items():
        ...
        if coords is not None:
            ...
            conv_img = cv2.resize(
                conv_img, (box_region.shape[1], box_region.shape[0]), interpolation=cv2.INTER_LINEAR
            )
...
def match_hp(image):
    ...
    while not matched:
        ...
        mask = cv2.resize(
            mask_orig, (int(w * factor), int(h * factor)), interpolation=cv2.INTER_LINEAR
        )
...
def ocr_hero_level_name(image) -> tuple[int, str]:
    ...
    matched = False
    while not matched:
        box_region = cv2.resize(
            box_region, (w * factor, h * factor), interpolation=cv2.INTER_LINEAR
        )
