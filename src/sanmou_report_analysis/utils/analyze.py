...
class UnitStatus:
    def __init__(self, team="", **kwargs):
        ...
        self.n_red = kwargs.get("n_red")
        self.兵力最大值 = kwargs.get("initial_hp")
        self.兵力 = kwargs.get("initial_hp")
        self.supply = 100
...
def analysis(report_id, config, data):
    ...
    for team_name, team_data in config.items():
        ...
        for _idx, general_data in team_data.items():
    ...
    for round_id, items in data.items():
        for sentence in items:
            ...
            if isinstance(sentence, Supply):
                ...
                current_initiator = general_tag
            elif isinstance(sentence, (DamageIncrease, DamageReduce)):
                ...
                if current_initiator is not None:
                    ...
                    record["发动武将武力"] = getattr(
                        general_dict.get(current_initiator), "武力", -1
                    )
                    record["发动武将智力"] = getattr(
                        general_dict.get(current_initiator), "智力", -1
                    )
                    record["发动武将统率"] = getattr(
                        general_dict.get(current_initiator), "统率", -1
                    )
                    record["发动武将先攻"] = getattr(
                        general_dict.get(current_initiator), "先攻", -1
                    )

                    whos_skill = None
                    skill_name = current_skill.split("-")[0]
                    # print(f"Current: {current_initiator}")
                    if skill_name in general_dict[current_initiator].skills:
                        whos_skill = general_dict[current_initiator].skills[skill_name]
                    elif skill_name in general_dict[general_tag].skills:
                ...
                other = (
                    1
                    + percent_to_float(getattr(general_dict[current_initiator], "造成治疗效果", 0))
                    + percent_to_float(getattr(general_dict[general_tag], "受到治疗增加", 0))
                )
                record = {}
                record["回合"] = round_id
                ...
                general_dict[general_tag].兵力 = tags[-1][0]
                # print(f"兵力变化记录: {heal_log[-1]}")

            elif isinstance(sentence, SkillDamage):
                general_dict[general_tag].兵力 = tags[4][0]
                record = {}
                record["回合"] = round_id
                record["受影响武将"] = general_tag
                ...
                general_dict[general_tag].兵力 = tags[5][0]
                record = {}
                record["回合"] = round_id
                record["受影响武将"] = general_tag
                record["发动武将"] = f"{tags[1][0]}_{tags[1][1]}"
                ...
                general_dict[general_tag].兵力 = tags[2][0]
                record = {}
                record["回合"] = round_id
                record["受影响武将"] = general_tag
                record["发动武将"] = current_initiator
