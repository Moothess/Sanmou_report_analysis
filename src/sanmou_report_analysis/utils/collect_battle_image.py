    ...
    for team, box_hero in _hero_layout.items():
        for idx in range(3):
            ...
            for skill_idx in range(3):
                skill_box = np.array([l2, t2, l2, t2]) + np.array(_hero_detail_layout["skill"]).copy()
                ...
                if check_normal_attack(skill_patch):
                    print(f"{team}队第{idx+1}个武将的第{skill_idx+1}个技能检测到普攻，跳过该武将")
                    break

                save_image(skill_patch, save_dir / "hero_detail" / f'{hero_tag}_{skill_idx+1}.png')
                ...
                save_image(skill_image, save_dir / "hero_detail" / f'{hero_tag}_skill{skill_idx+1}.png')
...
def collect_battle_detail(max_n=1000) -> list[np.ndarray]:
    ...
    for i in range(max_n):
        ...
        if report_imgs:
            match_result, good_count = is_image_similar_sift(image_summary, report_imgs[-1], ratio=0.05, min_match_count=500)
            if match_result:
                break
...
def get_detail_button_position():
    ...
    left = int(process_info['absolute_x'] + l)
