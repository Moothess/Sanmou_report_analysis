...
from sanmou_report_analysis.utils.image import are_images_matching, read_image
...
def get_battle_center(save_dir):
    ...
    for region in regions:
        ...
        if len_region > y_len * 0.10:
            ...
            cent = center * 1.0 / y_len * 0.78 + 0.17
            res.append(cent)
            if len(res) >= 2:
                break
    ...
    for battle in info_dict:
        ...
        for team_name, team_info in battle.items():
            for hero_idx, hero_info in team_info.items():
                ...
                info_dict_row[prefix + "国家"] = country
                info_dict_row[prefix + "等级"] = level
...
def collect_battle(save_dir, max_n=3, position_info=None):
    ...
    for i in range(max_n):
        ...
        # 进入战报详细界面
        abs_x, abs_y = convert_relative_xy_to_absolute(0.0, click_center)
        pyautogui.click(x=abs_x, y=abs_y)
        time.sleep(0.5)
