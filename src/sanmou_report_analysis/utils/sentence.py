...
def get_text_between(text, start_str, end_str):
    try:
        ...
        print(
            "无法从文本中提取指定内容: "
            f"{text}，起始标志: {start_str}，结束标志: {end_str}。尝试删除标志后提取。"
        )
...
class SkillEffectDamage(LineSentence):
    ...
    def match(color_list):
        return (
            calculate_similarity(color_list[3][0], "的效果损失了兵力")
            + calculate_similarity(color_list[5][0], "", "无法")
        ) / 2.0
...
class NormalHitFail(LineSentence):
    ...
    def match(color_list):
        return (
            calculate_similarity(color_list[3][0], "的效果")
            + calculate_similarity(color_list[5][0], "无法进行")
        ) / 2.0

    def print_line(self):
        general_tag = f"{self.sentence[0][0]}_{self.sentence[0][1]}"
        print(
            f"{general_tag}由于{self.sentence[1][0]}【{self.sentence[2][0]}】的「{self.sentence[3][0]}」效果[{self.sentence[4][0]}]无法进行{self.sentence[5][0]}"
        )
