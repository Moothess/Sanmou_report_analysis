...
def split_line_by_color(line_img):
    ...
    for text_color in [TextColor.BLUE, TextColor.RED]:
        ...
        if nonzero_pts is not None:
            ...
            if ww < 10:
                ...
                #raise RuntimeError(f"检测到宽度过小的颜色块，请检查图片: {debug_dir / f'line_{xw}_{xw+ww}_{text_color.value}_img.png'}")
    ...
    colored_segments = []
    for iii, pidx in enumerate(patch_idx):
    ...
    if len(colored_segments) > 6:
        ...
        raise RuntimeError(f"检测到过多的颜色块，可能是识别错误，请检查图片: {debug_dir / f'line_{xw}_{xw+ww}_{text_color.value}_img.png'}")
...
def split_image_to_lines(panorama):
    ...
    for row_idx in range(len(row_hist)):
        ...
        if top == -1 and row_hist[row_idx] > 30:
            top = row_idx
        elif top > -1 and (((row_hist[row_idx-1] - row_hist[row_idx]) > 80) or (row_hist[row_idx] < 30)):
            btm = row_idx
            # 两行记录拼接在一起了，需要拆分
            if btm - top > 35:
    ...
    # 逐行分析
    sttt = 0 # 4900
    for seg_idx, (seg_top, seg_btm) in enumerate(segment_idxs[sttt:]):
        ...
        if check_patch_with_color(seg_img[:, start:first_end + 1], TextColor.ICON) > 0.5:
            ...
            if round not in action_list:
                ...
                if len(segs) == 2: 
                    if segs[0][2] == TextColor.WHITE and segs[1][2] == TextColor.YELLOW:
                        # 特殊情况：行动顺序判定
                        action_list.append(round)
                        continue
            ...
            if start > img_width * 0.35 and start < img_width * 0.5:
                ...
                if round in stage_dict.keys():
                    ...
                    if height_diff > 0:
                        ...
                        first_line = np.pad(first_line, ((pad_top, pad_bottom), (0, 0), (0, 0)), mode='constant', constant_values=0)
                    elif height_diff < 0:
                        pad_top = (-height_diff) // 2
                        pad_bottom = -height_diff - pad_top
                        seg_img = np.pad(seg_img, ((pad_top, pad_bottom), (0, 0), (0, 0)), mode='constant', constant_values=0)
...
def get_text_from_patch(seg_img, start, end, text_color, hero_names, scale=2.0):
    ...
    nonzero_coords = cv2.findNonZero(cv2.cvtColor(tag_img0, cv2.COLOR_BGR2GRAY) if tag_img0.ndim == 3 else tag_img0)
    ...
    if text_color in [TextColor.BLUE, TextColor.RED]:
        ...
        for pot_name, pot_img in potential_names.items():
            ...
            if end1 - start1 < 10:
                ...
                #print(tag_img0.shape, start1, end1)
                #raise RuntimeError(f"名字块宽度过小，可能是识别错误，请检查图片: {debug_dir / f'line_{start}_{end}_cut.png'}")
                return None
            ...
            pot_img = cv2.resize(pot_img, (tag_img1.shape[1], tag_img1.shape[0]), interpolation=cv2.INTER_LINEAR)
        ...
        while not ocr_success:
            if w < 11:
                tag_img = np.pad(tag_img0, ((19, 19), (7, 7), (0, 0)), mode='constant', constant_values=0)
            else:
            ...
            tag_img = cv2.resize(tag_img, (int(w0 * scale), int(h0 * scale)), interpolation=cv2.INTER_LINEAR)
...
def find_sentence(color_list):
    max_sim = 0
    best_sentence = None
    potential_sentences = [subclass for subclass in LineSentence.__subclasses__() if subclass.len_patch == len(color_list)]
...
def analysis_image_by_lines(panorama, hero_names):
    ...
    for round, seg_imgs in stage_dict.items():        
        ...
        for i, seg_img in enumerate(seg_imgs):
            ...
            if isinstance(sentence, LineSentence):
                ...
                # 句式识别失败，需要检查优化代码
                assert type(sentence) == str, '分析代码BUG，请检查！'
                print(sentence)
    ...
    # 具体方法为：将未匹配行的图像保存到指定文件夹，人工查看图像内容后，按照“文本内容_颜色;文本内容_颜色;...”的格式输入
    # 若输入文本有误则需要重新输入，直到输入的内容成功匹配句式
    if len(unmatched_lines) > 0:
        print('开始处理未匹配行的图像，请打开文件夹：', debug_dir)
    for key, img in unmatched_lines.items():
...
def image_to_report(battle_report_dir, meta_info):
    ...
    # 每个武将名字都采用识别，难免会出现错误。因此这里采用另一种方式，通过前面识别到的武将名，生成对应的图片，随后通过需识别图像与各生成图片之间的像素值差异最小的来确定武将名字
    hero_names = {'blue': {}, 'red': {}}
    for side, heroes in meta_info.items():
        for hero_idx, info in heroes.items():
